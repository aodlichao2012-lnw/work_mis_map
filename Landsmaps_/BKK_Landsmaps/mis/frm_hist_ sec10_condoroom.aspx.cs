...
{
    public partial class frm_hist__sec10_condoroom : System.Web.UI.Page
    {
        ...
        private DataTable get_data(string criteria = "")
        {
            ...
            if (criteria != "")
            {
                strsql = "SELECT * FROM public.sec10_condoroom  Where " + criteria + "   area_id = " + global_variable.user_group_id +
                      " LIMIT 100"; /*" and area_code = '" + global_variable.user_dept_id +*/
            }

            else
            {
                strsql = "SELECT * FROM public.sec10_condoroom  Where  area_id = " + global_variable.user_group_id +
               " LIMIT 100"; /*" and area_code = '" + global_variable.user_dept_id +*/
            }
        ...
        private string make_string_criteria()
        {
            ...
            if (txt_keyword.Text != "")
            {
                res = res + " reg_no like " + my_functions.pre_str("%" + txt_keyword.Text + "%") + " or condo_name like " + my_functions.pre_str("%" + txt_keyword.Text + "%") + " or room_no like " + my_functions.pre_str(" % " + txt_keyword.Text + " % ") + " and";
            }
