...
{
    public partial class frm_hist_sec10_parcel_shp : System.Web.UI.Page
    {
        ...
        private DataTable get_data(string criteria = "")
        {
            ...
            if (criteria != "")
            {
                strsql = "SELECT * FROM public.sec10_parcel_shapefile  Where " + criteria + "   area_id = " + global_variable.user_group_id +
                      " LIMIT 100"; /*" and area_code = '" + global_variable.user_dept_id +*/
            }

            else
            {
                strsql = "SELECT * FROM public.sec10_parcel_shapefile  Where  area_id = " + global_variable.user_group_id +
               " LIMIT 100"; /*" and area_code = '" + global_variable.user_dept_id +*/
            }
        ...
        private string make_string_criteria()
        {
            ...
            if (txt_keyword.Text != "")
            {
                res = res + "( land_no like " + my_functions.pre_str("%" + txt_keyword.Text + "%") + " or tam_nam_th like " + my_functions.pre_str("%" + txt_keyword.Text + "%") + ")  and";
            }
